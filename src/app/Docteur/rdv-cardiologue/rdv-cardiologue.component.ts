...
@Component({
  selector: 'app-rdv-cardiologue',
  templateUrl: './rdv-cardiologue.component.html',
  styleUrls: ['./rdv-cardiologue.component.css']
})
export class RdvCardiologueComponent implements OnInit {
  public rdvs: Rdv[] = [];
  ...
  navConsulation(){
    this.router.navigate(['listConsCardio'])
  }
  ...
  public getRdvs(): void{
    this.rdvService.findRdvBySpecialite('Cardiologue').subscribe(
      (response: Rdv[])=>{
        this.rdvs = response;
      },
      (error: HttpErrorResponse) =>{
