...
@Component({
  selector: 'app-rdv-pediatre',
  templateUrl: './rdv-pediatre.component.html',
  styleUrls: ['./rdv-pediatre.component.css']
})
export class RdvPediatreComponent implements OnInit {
  ...
  navConsulation(){
    this.router.navigate(['listConsPediatre'])
  }
  ...
  public getRdvs(): void{
    this.rdvService.findRdvBySpecialite('Pediatre').subscribe(
      (response: Rdv[])=>{
        this.rdvs = response;
      },
      (error: HttpErrorResponse) =>{
