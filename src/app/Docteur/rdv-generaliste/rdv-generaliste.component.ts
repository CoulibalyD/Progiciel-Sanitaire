...
@Component({
  selector: 'app-rdv-generaliste',
  templateUrl: './rdv-generaliste.component.html',
  styleUrls: ['./rdv-generaliste.component.css']
})
export class RdvGeneralisteComponent implements OnInit {
  ...
  public getRdvs(): void{
    this.rdvService.findRdvBySpecialite('Generaliste').subscribe(
      (response: Rdv[])=>{
        this.rdvs = response;
      },
      (error: HttpErrorResponse) =>{
  ...
  public onAddRdv(addForm: NgForm): void{    
   ...
   navConsulation(){
    this.router.navigate(['listConsGeneralist'])
  }
