...
@Component({
  selector: 'app-rdv-ophtalmologue',
  templateUrl: './rdv-ophtalmologue.component.html',
  styleUrls: ['./rdv-ophtalmologue.component.css']
})
export class RdvOphtalmologueComponent implements OnInit {
  ...
  public getRdvs(): void{
    this.rdvService.findRdvBySpecialite('Ophtalmologue').subscribe(
      (response: Rdv[])=>{
        this.rdvs = response;
      },
      (error: HttpErrorResponse) =>{
  ...
  public onAddRdv(addForm: NgForm): void{    
   ...
   navConsulation(){
    this.router.navigate(['listConsOphtalmo'])
  }
